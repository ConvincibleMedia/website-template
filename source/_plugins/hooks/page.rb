Jekyll::Hooks.register :site, :pre_render do |site, payload|
	...
	# SEO Separator
	sep = ""
	expect(expect_key(site.data, ['siteinfo','seo','separator']), String) { |e| sep = e.strip }
	...
	site.collections.each { |collection| # Collections are presented as array [label, collection]
		...
		collection[1].docs.each { |page|
				...
				# META TITLE
				page_title = "Page"
				if expect(expect_key(page.data, ['seo', 'title']), String) { |e| page_title = e }
				elsif expect(page.data['title'], String) { |e| page_title = e }
				elsif expect(expect_key(site.data, ['siteinfo','seo','title']), String) { |e| page_title = e }
				end
				page_meta_title = page_title
				if expect(page.data['parents'], Array)
					page.data['parents'].each { |parent|
						unless expect_key(site.data, ['siteinfo','seo','ignore']).include?(parent)
							page_meta_title += sep + parent.title
						end
					}
				end
				if !page_home then expect(expect_key(site.data, ['siteinfo','title']), String) { |e| page_meta_title += sep + e } end
				expect(expect_text(expect_key(site.data, ['siteinfo','seo','suffix'])), String) { |e| page_meta_title += sep + e }

				# META DESCRIPTION
				page_meta_description = ""
				if expect(expect_key(page.data, ['seo','description']), String) { |e| page_meta_description = e }
				elsif expect(expect_key(page.data, ['excerpt']), String) { |e| page_meta_description = e } # If defined explicitly in YAML
				elsif expect(expect_key(page.data, ['excerpt']), Jekyll::Excerpt) { |e| # Generated by Jekyll (but we switch this off - below code is in case we ever decide to switch it back on!)
